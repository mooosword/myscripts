...
Date:   20140915
Version:    0.03  
Note:  add function debug, fix a decide_bin_size bug
'''
...
def draw_distribution_chart(group_dic, max_dic, cover_dic, feature, nbins, bin_size,  output_path):
    ...
    plt.legend( tuple( [ tmp[0] for tmp in res_list]), tuple( [gname for gname in sorted(group_dic.keys()) ]) ) 
...
def decide_bin_size(data1, data2, funcdebug=False):
    ...
    if funcdebug:
        print "\t\t[Func Debug] nmax = %d" % nmax
    raw_bin_dic, raw_bin_size = get_bin_dic(data, BIN_NUM, nmax)
   
    if funcdebug:
        print "\t\t[Func Debug] raw_bin_size = %d" % raw_bin_size
    ...
    if funcdebug:
        print "\t\t[Func Debug] bin_size = %d" % bin_size
...
def gather_data(input1, input2, feature, bin_size, bin_num, is_normalize):
    ...
    if not bin_size:
        ...
        bin_size = decide_bin_size(data1, data2, funcdebug=True)
        if debug:
            print "\t[DEBUG] [bin size = %f]" % bin_size
